...
class Holiday_logic
{
    ...
    public function get_work_date(int $before_days, string $now_date, $type = 1)
    {
        ...
        if (!empty($before_days) && \is_int($before_days) && !empty($now_date)) {
            ...
            while ($limit > 0) {
                ...
                if (!\in_array(date('Y-m-d', $day), $Holiday_array, true) && !\in_array((int) date('w', $day), [0, 6], true)) {
                    --$limit;
                }
    ...
    public function is_holiday(int $date)
    {
        ...
        try {
            ...
            if (\in_array(date('Y-m-d', $date), $Holiday_array, true)) {
                throw new \Exception(true);
            }

            if (\in_array((int) date('w', $date), [0, 6], true)) {
                throw new \Exception(true);
            }
        } catch (\Exception $e) {
            $result = true;
