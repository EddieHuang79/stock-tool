...
class FindStrategy
{
    ...
    private function set()
    {
        ...
        foreach ($this->year as $year) 
        {
            ...
            $this->stock_data[$year] = collect($this->stock_data[$year])->map(function($item) {
                return collect($item)->mapWithKeys(function($item) {
                    return [$item->data_date => collect($item)->toArray()];
                })->toArray();
            })->toArray();
            $this->bollingerBandsData[$year] = $this->Tech->get_data_by_year( $year, $this->profit_data_filter[$year] );
        }
        ...
        foreach ($this->year as $year) 
        {
            ...
            $this->profit_data_filter[$year] = collect($this->profit_data)->filter(function($item) use($year) {
                $eps = collect($item["eps"])->filter(function($item, $key) use($year) {
                    return in_array($key, [$year - 3, $year - 2, $year - 1]);
                });
        ...
        if (isset($fund_data[$assignDate])) 
        {
            ...
            foreach ($fund_data as $dateKey => $item) 
            {
                ...
                if (strtotime($dateKey) <= strtotime($assignDate)) 
                {
                    ...
                    $result = intval($item[$column]) > 0 ? $result + 1 : 0;
