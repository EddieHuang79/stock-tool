...
class JackStrategySimulation1 extends JackStrategy
{
        ...
        foreach ($this->year as $year) 
        {
            ...
            $this->volume_data[$year]->filter(function($item, $year) {
                ...
                foreach ($item as $date => $volume) 
                {
                    ...
                    $investmentTrust_overDays = $this->count_fund_over_days($year, $stock_id, $date, "investment_trust_total");
                    ...
                    if ($volume > 500 && $over_MA60_days["days"] <= 5 && $over_MA20_days["days"] <= 3 && $over_MA60_days["status"] && $over_MA20_days["status"] && $has_stock === false && $investmentTrust_overDays > 1) 
                    {
    ...
    public function do(array $year)
    {
        ...
        $this->set_file_name( "Strategy/JackStrategySimulation1-2.txt" );

        $this->set_log_title( "JackStrategySimulation1-2" );
